...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-divid.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "divid")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of divid binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-divid.conf")

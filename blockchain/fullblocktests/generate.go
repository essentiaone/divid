...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: diviutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func makeTestGenerator(params *chaincfg.Params) (testGenerator, error) {
	privKey, _ := diviec.PrivKeyFromBytes(diviec.S256(), []byte{0x01})
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := diviutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*diviutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, diviutil.NewTx(tx))
	}
...
func additionalCoinbase(amount diviutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee diviutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee diviutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee diviutil.Amount) *wire.MsgTx {
...
func encodeNonCanonicalBlock(b *wire.MsgBlock) []byte {
	...
	b.Header.DiviEncode(&buf, 0, wire.BaseEncoding)
	buf.Write(nonCanonicalVarInt(uint32(len(b.Transactions))))
	for _, tx := range b.Transactions {
		tx.DiviEncode(&buf, 0, wire.BaseEncoding)
	}
	return buf.Bytes()
}

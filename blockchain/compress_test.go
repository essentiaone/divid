...
func TestAmountCompression(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "0 DIVI (sometimes used in nulldata)",
			uncompressed: 0,
			compressed:   0,
		},
		{
		...
		{
			name:         "0.00001 DIVI (typical transaction fee)",
			uncompressed: 1000,
			compressed:   4,
		},
		{
			name:         "0.0001 DIVI (typical transaction fee)",
			uncompressed: 10000,
			compressed:   5,
		},
		{
			name:         "0.12345678 DIVI",
		...
		{
			name:         "21000000 DIVI (max minted coins)",
...
func TestCompressedTxOut(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "pay-to-pubkey uncompressed 1 DIVI",
			amount:     100000000,

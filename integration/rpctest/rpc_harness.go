...
// Harness fully encapsulates an active divid process to provide a unified
// platform for creating rpc driven integration tests involving divid. The
// active divid node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active divid process is fully
...
func New(activeNet *chaincfg.Params, handlers *rpcclient.NotificationHandlers,
	...
	if handlers.OnFilteredBlockConnected != nil {
		...
		handlers.OnFilteredBlockConnected = func(height int32, header *wire.BlockHeader, filteredTxns []*diviutil.Tx) {
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the divid node itself. This spawns a new process which will be
	// managed
	...
	// wallet.
	filterAddrs := []diviutil.Address{h.wallet.coinbaseAddr}
	if err := h.Node.LoadTxFilter(true, filterAddrs, nil); err != nil {
		return err
	}

	// Ensure divid properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created divid
...
func (h *Harness) NewAddress() (diviutil.Address, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) ConfirmedBalance() diviutil.Amount {
...
func (h *Harness) SendOutputs(targetOutputs []*wire.TxOut,
	feeRate diviutil.Amount) (*chainhash.Hash, error) {
...
func (h *Harness) CreateTransaction(targetOutputs []*wire.TxOut,
	feeRate diviutil.Amount, change bool) (*wire.MsgTx, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) GenerateAndSubmitBlock(txns []*diviutil.Tx, blockVersion int32,
	blockTime time.Time) (*diviutil.Block, error) {
...
func (h *Harness) GenerateAndSubmitBlockWithCustomCoinbaseOutputs(
	txns []*diviutil.Tx, blockVersion int32, blockTime time.Time,
	mineTo []wire.TxOut) (*diviutil.Block, error) {
	...
	prevBlock := diviutil.NewBlock(mBlock)
...
func baseDir() (string, error) {
	dirPath := filepath.Join(os.TempDir(), "divid", "rpctest")

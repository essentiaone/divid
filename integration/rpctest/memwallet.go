...
type memWallet struct {
	coinbaseKey  *diviec.PrivateKey
	coinbaseAddr diviutil.Address
	...
	addrs map[uint32]diviutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]diviutil.Address)
...
// SetRPCClient saves the passed rpc connection to divid as the wallet's
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*diviutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          diviutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (diviutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []diviutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt diviutil.Amount,
	feeRate diviutil.Amount, change bool) error {
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := diviutil.Amount(txSize * int(feeRate))
...
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut,
	...
	var outputAmt diviutil.Amount
	for _, output := range outputs {
		outputAmt += diviutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() diviutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance diviutil.Amount
...
func keyToAddr(key *diviec.PrivateKey, net *chaincfg.Params) (diviutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := diviutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}

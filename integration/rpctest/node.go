...
import (
	...
	rpc "github.com/DiviProject/divid/rpcclient"
	"github.com/DiviProject/diviutil"
)

// nodeConfig contains all the args, and data required to launch a divid process
...
func newConfig(prefix, certFile, keyFile string, extra []string) (*nodeConfig, error) {
	dividPath, err := dividExecutablePath()
	if err != nil {
		dividPath = "divid"
	}
...
// arguments returns an array of arguments that be used to launch the divid
// process.
...
// command returns the exec.Cmd which will be used to start the divid process.
...
// to the divid process that is launched via Start().
...
// as the base for the log and data directories for divid.
...
// start creates a new divid process, and writes its pid in a file reserved for
...
// stop interrupts the running divid process process, and waits until it exits
...
// shutdown terminates the running divid process, and cleans up all
...
func genCertPair(certFile, keyFile string) error {
	...
	cert, key, err := diviutil.NewTLSCertPair(org, validUntil, nil)
	if err != nil {
		return err
	}

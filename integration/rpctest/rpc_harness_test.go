...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt diviutil.Amount) *chainhash.Hash {
		...
		// Next, send amt DIVI to this address, spending from one of our mature
		// coinbase outputs.
	...
	// input.
	txid := genSpend(diviutil.Amount(5 * diviutil.SatoshiPerBitcoin))
	...
	txid = genSpend(diviutil.Amount(500 * diviutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(diviutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*diviutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, diviutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	// The internal wallet of this harness should now have 250 DIVI.
	expectedBalance := diviutil.Amount(250 * diviutil.SatoshiPerBitcoin)
	...
	// The original wallet should now have a balance of 0 DIVI as its entire
	// chain should have been decimated in favor of the main harness'
	// chain.
	expectedBalance = diviutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := diviutil.Amount(50 * diviutil.SatoshiPerBitcoin)
	...
	// The current wallet balance should now be at least 50 DIVI less
...
func TestHarness(t *testing.T) {
	// We should have (numMatureOutputs * 50 DIVI) of mature unspendable
	// outputs.
	expectedBalance := diviutil.Amount(numMatureOutputs * 50 * diviutil.SatoshiPerBitcoin)

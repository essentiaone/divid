...
import (
	...
	"github.com/DiviProject/divid/integration/rpctest"
...
func makeTestOutput(r *rpctest.Harness, t *testing.T,
	amt diviutil.Amount) (*diviec.PrivateKey, *wire.OutPoint, []byte, error) {
	...
	// spend.
	a, err := diviutil.NewAddressPubKey(key.PubKey().SerializeCompressed(), r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0113Activation(t *testing.T) {
	...
	dividCfg := []string{"--rejectnonstd"}
	r, err := rpctest.New(&chaincfg.SimNetParams, nil, dividCfg)
	if err != nil {
	...
	const outputValue = diviutil.SatoshiPerBitcoin
	...
	txns := []*diviutil.Tx{diviutil.NewTx(tx)}
...
func createCSVOutput(r *rpctest.Harness, t *testing.T,
	numSatoshis diviutil.Amount, timeLock int32,
	...
	p2shAddr, err := diviutil.NewAddressScriptHash(csvScript, r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0068AndBIP0112Activation(t *testing.T) {
	...
	const (
		outputAmt         = diviutil.SatoshiPerBitcoin
	...
	for txVersion := int32(0); txVersion < 3; txVersion++ {
		...
		txns := []*diviutil.Tx{diviutil.NewTx(spendingTx)}
		block, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
		if err != nil {
			t.Fatalf("unable to submit block: %v", err)
		}
	...
	for i, test := range tests {
		...
		if !test.accept {
			txns := []*diviutil.Tx{diviutil.NewTx(test.tx)}
			_, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
			if err == nil {

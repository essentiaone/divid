...
var (
	...
	btcctlHomeDir         = diviutil.AppDataDir("btcctl", false)
	btcwalletHomeDir      = diviutil.AppDataDir("btcwallet", false)
	defaultConfigFile     = filepath.Join(btcctlHomeDir, "btcctl.conf")
	defaultRPCServer      = "localhost"
	defaultRPCCertFile    = filepath.Join(dividHomeDir, "rpc.cert")
...
func loadConfig() (*config, []string, error) {
	...
	if _, err := os.Stat(preCfg.ConfigFile); os.IsNotExist(err) {
		...
		if preCfg.Wallet {
			...
			serverConfigPath = filepath.Join(dividHomeDir, "divid.conf")
		}
...
// For this it tries to read the config file for the RPC server (either divid or

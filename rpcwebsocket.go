...
import (
	...
	"github.com/DiviProject/divid/database"
...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *diviutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *diviutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *diviutil.Tx, isNew bool) {
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) addAddress(a diviutil.Address) {
	switch a := a.(type) {
...
func (f *wsClientFilter) addAddressStr(s string, params *chaincfg.Params) {
	...
	// script.
	a, err := diviutil.DecodeAddress(s, params)
	if err != nil {
		return
	}
	f.addAddress(a)
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) existsAddress(a diviutil.Address) bool {
	switch a := a.(type) {
	case *diviutil.AddressPubKeyHash:
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) removeAddress(a diviutil.Address) {
	switch a := a.(type) {
	case *diviutil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected diviutil.Block
type notificationBlockDisconnected diviutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *diviutil.Tx,
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *diviutil.Block) {
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *diviutil.Tx) {
	...
	ntfn := btcjson.NewTxAcceptedNtfn(txHashStr, diviutil.Amount(amount).ToDIVI())
	marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
	if err != nil {
...
func (m *wsNotificationManager) addSpentRequests(opMap map[wire.OutPoint]map[chan struct{}]*wsClient,
	...
	spends := make(map[chainhash.Hash]*diviutil.Tx)
	for _, op := range ops {
...
func blockDetails(block *diviutil.Block, txIndex int) *btcjson.BlockDetails {
	if block == nil {
		return nil
	}
...
func newRedeemingTxNotification(txHex string, index int, block *diviutil.Block) ([]byte, error) {
...
func (m *wsNotificationManager) notifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	addrs map[string]map[chan struct{}]*wsClient, tx *diviutil.Tx, block *diviutil.Block) {
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *diviutil.Tx,
...
func (m *wsNotificationManager) notifyForTxIns(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	tx *diviutil.Tx, block *diviutil.Block) {
...
out:
	for {
		...
		//
		// Divid does not respond to any request without and "id" or "id":null,
...
func checkAddressValidity(addrs []string, params *chaincfg.Params) error {
	for _, addr := range addrs {
		_, err := diviutil.DecodeAddress(addr, params)
		if err != nil {
...
func rescanBlock(wsc *wsClient, lookups *rescanKeys, blk *diviutil.Block) {
...
//
// NOTE: This extension is ported from github.com/decred/dcrd
func rescanBlockFilter(filter *wsClientFilter, block *diviutil.Block, params *chaincfg.Params) []string {
...
func descendantBlock(prevHash *chainhash.Hash, curBlock *diviutil.Block) error {
...
func scanBlockChunks(wsc *wsClient, cmd *btcjson.RescanCmd, lookups *rescanKeys, minBlock,
	...
	*diviutil.Block, *chainhash.Hash, error) {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	...
	// Notify websocket client of the finished rescan.  Due to how divid

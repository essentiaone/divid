...
const (
	...
	MinHighPriority = diviutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr diviutil.Address) (*diviutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *diviutil.Tx, height int32) error {
...
func logSkippedDeps(tx *diviutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress diviutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*diviutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := diviutil.NewTx(coinbaseTx.MsgTx().Copy())
	...
	block := diviutil.NewBlock(&msgBlock)
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A diviutil.Block should use saved in the state to avoid

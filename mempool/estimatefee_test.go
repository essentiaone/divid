...
func (eft *estimateFeeTester) testTx(fee diviutil.Amount) *TxDesc {
	eft.version++
	return &TxDesc{
		TxDesc: mining.TxDesc{
			Tx: diviutil.NewTx(&wire.MsgTx{
				Version: eft.version,
			}),
			Height: eft.height,
			Fee:    int64(fee),
...
func expectedFeePerKilobyte(t *TxDesc) DiviPerKilobyte {
	size := float64(t.TxDesc.Tx.MsgTx().SerializeSize())
	fee := float64(t.TxDesc.Fee)

	return SatoshiPerByte(fee / size).ToDiviPerKb()
}

func (eft *estimateFeeTester) newBlock(txs []*wire.MsgTx) {
	eft.height++

	block := diviutil.NewBlock(&wire.MsgBlock{
		Transactions: txs,
	})
...
func (eft *estimateFeeTester) estimates() [estimateFeeDepth]DiviPerKilobyte {

	// Generate estimates
	var estimates [estimateFeeDepth]DiviPerKilobyte
...
func (eft *estimateFeeTester) round(txHistory [][]*TxDesc,
	estimateHistory [][estimateFeeDepth]DiviPerKilobyte,
	txPerRound, txPerBlock uint32) ([][]*TxDesc, [][estimateFeeDepth]DiviPerKilobyte) {

	// generate new txs.
	var newTxs []*TxDesc
	for i := uint32(0); i < txPerRound; i++ {
		newTx := eft.testTx(diviutil.Amount(rand.Intn(1000000)))
...
func TestEstimateFeeRollback(t *testing.T) {
	...
	estimateHistory := [][estimateFeeDepth]DiviPerKilobyte{eft.estimates()}
...
func (eft *estimateFeeTester) checkSaveAndRestore(
	previousEstimates [estimateFeeDepth]DiviPerKilobyte) {

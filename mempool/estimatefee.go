...
// DiviPerKilobyte is number with units of bitcoins per kilobyte.
type DiviPerKilobyte float64

// ToDiviPerKb returns a float value that represents the given
// SatoshiPerByte converted to satoshis per kb.
func (rate SatoshiPerByte) ToDiviPerKb() DiviPerKilobyte {
	// If our rate is the error value, return that.
	if rate == SatoshiPerByte(-1.0) {
		return -1.0
	}

	return DiviPerKilobyte(float64(rate) * bytePerKb * btcPerSatoshi)
}

// Fee returns the fee for a transaction of a given size for
// the given fee rate.
func (rate SatoshiPerByte) Fee(size uint32) diviutil.Amount {
	// If our rate is the error value, return that.
	if rate == SatoshiPerByte(-1) {
		return diviutil.Amount(-1)
	}

	return diviutil.Amount(float64(rate) * float64(size))
}

// NewSatoshiPerByte creates a SatoshiPerByte from an Amount and a
// size in bytes.
func NewSatoshiPerByte(fee diviutil.Amount, size uint32) SatoshiPerByte {
...
func (ef *FeeEstimator) ObserveTransaction(t *TxDesc) {
	...
	if _, ok := ef.observed[hash]; !ok {
		...
		ef.observed[hash] = &observedTransaction{
			hash:     hash,
			feeRate:  NewSatoshiPerByte(diviutil.Amount(t.Fee), size),
			observed: t.Height,
...
func (ef *FeeEstimator) RegisterBlock(block *diviutil.Block) error {
	ef.mtx.Lock()
	defer ef.mtx.Unlock()
	...
	transactions := make(map[*diviutil.Tx]struct{})
...
func (ef *FeeEstimator) EstimateFee(numBlocks uint32) (DiviPerKilobyte, error) {
	ef.mtx.Lock()
	defer ef.mtx.Unlock()
	...
	return ef.cached[int(numBlocks)-1].ToDiviPerKb(), nil
}

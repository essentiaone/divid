...
const (
	...
	DefaultMinRelayTxFee = diviutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee diviutil.Amount) int64 {
	...
	if minFee < 0 || minFee > diviutil.MaxSatoshi {
		minFee = diviutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *diviutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee diviutil.Amount) bool {
...
func checkTransactionStandard(tx *diviutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee diviutil.Amount,
...
func GetTxVirtualSize(tx *diviutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4

...
5. [How do I use bootstrap.dat with divid?](#Importing)
...
**NOTE:** Using bootstrap.dat is entirely optional.  Divid will download the
...
### 5. How do I use bootstrap.dat with divid?

divid comes with a separate utility named `addblock` which can be used to import
...
1. Stop divid if it is already running.  This is required since addblock needs to
   access the database used by divid and it will be locked if divid is using it.
...
**Windows:**
```bat
C:\> "%PROGRAMFILES%\Divid Suite\Divid\addblock" -i C:\Path\To\bootstrap.dat
```

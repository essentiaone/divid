// Copyright (c) 2015-2017 The DiviProject developers
...
var helpDescsEnUS = map[string]string{
	...
	"debuglevel--synopsis": "Dynamically changes the debug logging level.\n" +
		...
		"The valid subsystems are AMGR, ADXR, BCDB, BMGR, DIVID, CHAN, DISC, PEER, RPCS, SCRP, SRVR, and TXMP.\n" +
	...
	"createrawtransaction-amounts--desc":  "The destination address as the key and the amount in DIVI as the value",
	...
	"getblock-verbosetx":   "Specifies that each transaction is returned as a JSON object and only applies if the verbose flag is true (divid extension)",
	...
	"infochainresult-relayfee":        "The minimum relay fee for non-free transactions in DIVI/KB",
	...
	"infowalletresult-paytxfee":        "The transaction fee set in DIVI/KB",
	"infowalletresult-relayfee":        "The minimum relay fee for non-free transactions in DIVI/KB",
	...
	"gettxoutresult-value":         "The transaction amount in DIVI",
	...
	"sendrawtransaction-allowhighfees": "Whether or not to allow insanely high fees (divid does not yet implement this parameter, so it has no effect)",
	...
	// StopCmd help.
	"stop--synopsis": "Shutdown divid.",
	"stop--result0":  "The string 'divid stopping.'",
	...
	// NotifySpentCmd help.
	"notifyspent--synopsis": "Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this divid instance) and when such a transaction first appears in a newly-attached block.",

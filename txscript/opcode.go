...
func opcodeCheckSig(op *parsedOpcode, vm *Engine) error {
	...
	pubKey, err := diviec.ParsePubKey(pkBytes, diviec.S256())
	if err != nil {
		vm.dstack.PushBool(false)
		return nil
	}

	var signature *diviec.Signature
	if vm.hasFlag(ScriptVerifyStrictEncoding) ||
		vm.hasFlag(ScriptVerifyDERSignatures) {

		signature, err = diviec.ParseDERSignature(sigBytes, diviec.S256())
	} else {
		signature, err = diviec.ParseSignature(sigBytes, diviec.S256())
	}
	if err != nil {
		vm.dstack.PushBool(false)
		return nil
...
type parsedSigInfo struct {
	signature       []byte
	parsedSignature *diviec.Signature
	parsed          bool
}
...
func opcodeCheckMultiSig(op *parsedOpcode, vm *Engine) error {
	...
	for numSignatures > 0 {
		...
		var parsedSig *diviec.Signature
		if !sigInfo.parsed {
			if err := vm.checkHashTypeEncoding(hashType); err != nil {
				return err
			}
			...
			if vm.hasFlag(ScriptVerifyStrictEncoding) ||
				...
				parsedSig, err = diviec.ParseDERSignature(signature,
					diviec.S256())
			} else {
				parsedSig, err = diviec.ParseSignature(signature,
					diviec.S256())
			}
			sigInfo.parsed = true
			if err != nil {
		...
		// Parse the pubkey.
		parsedPubKey, err := diviec.ParsePubKey(pubKey, diviec.S256())
		if err != nil {
			continue
		}

...
// newAddressPubKey returns a new diviutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) diviutil.Address {
	addr, err := diviutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new diviutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) diviutil.Address {
	addr, err := diviutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new diviutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) diviutil.Address {
	addr, err := diviutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the diviutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the diviutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := diviutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := diviutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := diviutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := diviutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := diviutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*diviutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*diviutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*diviutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*diviutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*diviutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,

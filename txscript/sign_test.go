...
type addressToKey struct {
	key        *diviec.PrivateKey
	compressed bool
}

func mkGetKey(keys map[string]addressToKey) KeyDB {
	if keys == nil {
		return KeyClosure(func(addr diviutil.Address) (*diviec.PrivateKey,
			bool, error) {
...
func mkGetScript(scripts map[string][]byte) ScriptDB {
	if scripts == nil {
		return ScriptClosure(func(addr diviutil.Address) ([]byte, error) {
...
func TestSignTxOutput(t *testing.T) {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			key, err := diviec.NewPrivateKey(diviec.S256())
			if err != nil {
			...
			pk := (*diviec.PublicKey)(&key.PublicKey).
				SerializeUncompressed()
			address, err := diviutil.NewAddressPubKeyHash(
				diviutil.Hash160(pk), &chaincfg.TestNet3Params)
			if err != nil {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			pk := (*diviec.PublicKey)(&key.PublicKey).
				SerializeUncompressed()
			address, err := diviutil.NewAddressPubKey(pk,
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			scriptAddr, err := diviutil.NewAddressScriptHash(
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			key1, err := diviec.NewPrivateKey(diviec.S256())
			if err != nil {
				t.Errorf("failed to make privKey for %s: %v",
					msg, err)
				break
			...
			pk1 := (*diviec.PublicKey)(&key1.PublicKey).
				SerializeCompressed()
			address1, err := diviutil.NewAddressPubKey(pk1,
				&chaincfg.TestNet3Params)
			if err != nil {
				t.Errorf("failed to make address for %s: %v",
					msg, err)
			...
			key2, err := diviec.NewPrivateKey(diviec.S256())
			if err != nil {
			...
			pk2 := (*diviec.PublicKey)(&key2.PublicKey).
				SerializeCompressed()
			address2, err := diviutil.NewAddressPubKey(pk2,
				&chaincfg.TestNet3Params)
			if err != nil {
			...
			pkScript, err := MultiSigScript(
				[]*diviutil.AddressPubKey{address1, address2},
				2)
			if err != nil {
				t.Errorf("failed to make pkscript "+
					"for %s: %v", msg, err)
...
func TestSignatureScript(t *testing.T) {
	...
	privKey, _ := diviec.PrivKeyFromBytes(diviec.S256(), privKeyD)

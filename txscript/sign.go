...
func RawTxInWitnessSignature(tx *wire.MsgTx, sigHashes *TxSigHashes, idx int,
	...
	key *diviec.PrivateKey) ([]byte, error) {
...
// WitnessSignature creates an input witness stack for tx to spend DIVI sent
...
func WitnessSignature(tx *wire.MsgTx, sigHashes *TxSigHashes, idx int, amt int64,
	subscript []byte, hashType SigHashType, privKey *diviec.PrivateKey,
	...
	pk := (*diviec.PublicKey)(&privKey.PublicKey)
	var pkData []byte
	if compress {
...
func RawTxInSignature(tx *wire.MsgTx, idx int, subScript []byte,
	hashType SigHashType, key *diviec.PrivateKey) ([]byte, error) {
...
// SignatureScript creates an input signature script for tx to spend DIVI sent
...
func SignatureScript(tx *wire.MsgTx, idx int, subscript []byte, hashType SigHashType, privKey *diviec.PrivateKey, compress bool) ([]byte, error) {
...
func p2pkSignatureScript(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType, privKey *diviec.PrivateKey) ([]byte, error) {
	sig, err := RawTxInSignature(tx, idx, subScript, hashType, privKey)
	if err != nil {
		return nil, err
	}
...
func signMultiSig(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType,
	addresses []diviutil.Address, nRequired int, kdb KeyDB) ([]byte, bool) {
...
func sign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	ScriptClass, []diviutil.Address, int, error) {
...
func mergeScripts(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	pkScript []byte, class ScriptClass, addresses []diviutil.Address,
...
func mergeMultiSig(tx *wire.MsgTx, idx int, addresses []diviutil.Address,
...
sigLoop:
	for _, sig := range possibleSigs {
		...
		pSig, err := diviec.ParseDERSignature(tSig, diviec.S256())
		if err != nil {
			continue
		}
		...
		for _, addr := range addresses {
			...
			// bad input.
			pkaddr := addr.(*diviutil.AddressPubKey)
...
type KeyDB interface {
	GetKey(diviutil.Address) (*diviec.PrivateKey, bool, error)
}

// KeyClosure implements KeyDB with a closure.
type KeyClosure func(diviutil.Address) (*diviec.PrivateKey, bool, error)

// GetKey implements KeyDB by returning the result of calling the closure.
func (kc KeyClosure) GetKey(address diviutil.Address) (*diviec.PrivateKey,
	bool, error) {
	return kc(address)
}
...
type ScriptDB interface {
	GetScript(diviutil.Address) ([]byte, error)
}

// ScriptClosure implements ScriptDB with a closure.
type ScriptClosure func(diviutil.Address) ([]byte, error)

// GetScript implements ScriptDB by returning the result of calling the closure.
func (sc ScriptClosure) GetScript(address diviutil.Address) ([]byte, error) {
	return sc(address)
}

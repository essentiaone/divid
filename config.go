...
import (
	...
	"github.com/DiviProject/divid/connmgr"
	"github.com/DiviProject/divid/database"
	_ "github.com/DiviProject/divid/database/ffldb"
...
const (
	defaultConfigFilename        = "divid.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "divid.log"
	defaultMaxPeers              = 125
	...
	sampleConfigFilename         = "sample-divid.conf"
	defaultTxIndex               = false
	defaultAddrIndex             = false
)

var (
	defaultHomeDir     = diviutil.AppDataDir("divid", false)
...
// config defines the configuration options for divid.
//
...
type config struct {
	...
	AgentBlacklist       []string      `long:"agentblacklist" description:"A comma separated list of user-agent substrings which will cause divid to reject any peers whose user-agent contains any of the blacklisted substrings."`
	AgentWhitelist       []string      `long:"agentwhitelist" description:"A comma separated list of user-agent substrings which will cause divid to require all peers' user-agents to contain one of the whitelisted substrings. The blacklist is applied before the blacklist, and an empty whitelist will allow all agents that do not fail the blacklist."`
	...
	MinRelayTxFee        float64       `long:"minrelaytxfee" description:"The minimum transaction fee in DIVI/kB to be considered a non-zero fee."`
...
//
// The above results in divid functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	cfg := config{
		...
		MinRelayTxFee:        mempool.DefaultMinRelayTxFee.ToDIVI(),
	...
	if cfg.DisableRPC {
		dividLog.Infof("RPC service is disabled")
	}
	...
	cfg.minRelayTxFee, err = diviutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]diviutil.Address, 0, len(cfg.MiningAddrs))
	for _, strAddr := range cfg.MiningAddrs {
		addr, err := diviutil.DecodeAddress(strAddr, activeNetParams.Params)
		if err != nil {
	...
	if configFileError != nil {
		dividLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// createDefaultConfig copies the file sample-divid.conf to the given destination path,
...
// dividDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func dividDial(addr net.Addr) (net.Conn, error) {
...
// dividLookup resolves the IP of the given host using the correct DNS lookup
...
func dividLookup(host string) ([]net.IP, error) {

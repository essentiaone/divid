...
import (
	...
	"github.com/DiviProject/divid/addrmgr"
	...
	"github.com/DiviProject/divid/mining/cpuminer"
	"github.com/DiviProject/divid/netsync"
...
func (sp *serverPeer) OnTx(_ *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a diviutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := diviutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(_ *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a diviutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := diviutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (sp *serverPeer) OnFeeFilter(_ *peer.Peer, msg *wire.MsgFeeFilter) {
	...
	if msg.MinFee < 0 || msg.MinFee > diviutil.MaxSatoshi {
		peerLog.Debugf("Peer %v sent an invalid feefilter '%v' -- "+
			"disconnecting", sp, diviutil.Amount(msg.MinFee))
		sp.Disconnect()
		return
	}
...
func (s *server) TransactionConfirmed(tx *diviutil.Tx) {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices,
			dividLookup, func(addrs []*wire.NetAddress) {
...
// newServer returns a new divid server configured to listen on addr for the
...
func newServer(listenAddrs, agentBlacklist, agentWhitelist []string,
	...
	amgr := addrmgr.New(cfg.DataDir, dividLookup)
	...
	txC := mempool.Config{
		...
		CalcSequenceLock: func(tx *diviutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {

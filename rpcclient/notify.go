...
type NotificationHandlers struct {
	...
	OnRecvTx func(transaction *diviutil.Tx, details *btcjson.BlockDetails)
	...
	//
	// NOTE: Deprecated. Use OnRelevantTxAccepted instead.
	OnRedeemingTx func(transaction *diviutil.Tx, details *btcjson.BlockDetails)
	...
	// result of a rescan request, due to how divid may send various rescan
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount diviutil.Amount)
	...
	// OnDividConnected is invoked when a wallet connects or disconnects from
	// divid.
	//
	// This will only be available when client is connected to a wallet
	// server such as btcwallet.
	OnDividConnected func(connected bool)

	// OnAccountBalance is invoked with account balance updates.
	//
	// This will only be available when speaking to a wallet server
	// such as btcwallet.
	OnAccountBalance func(account string, balance diviutil.Amount, confirmed bool)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	// OnDividConnected
	case btcjson.DividConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
		if c.ntfnHandlers.OnDividConnected == nil {
			return
		}

		connected, err := parseDividConnectedNtfnParams(ntfn.Params)
		if err != nil {
			log.Warnf("Received invalid divid connected "+
				"notification: %v", err)
			return
		}

		c.ntfnHandlers.OnDividConnected(connected)
...
//
// NOTE: This is a divid extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func parseFilteredBlockConnectedParams(params []json.RawMessage) (int32,
	*wire.BlockHeader, []*diviutil.Tx, error) {
	...
	transactions := make([]*diviutil.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = diviutil.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
// notifications.
func parseChainTxNtfnParams(params []json.RawMessage) (*diviutil.Tx,
	*btcjson.BlockDetails, error) {
	...
	return diviutil.NewTx(&msgTx), block, nil
}
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := diviutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
// parseDividConnectedNtfnParams parses out the connection status of divid
// and btcwallet from the parameters of a dividconnected notification.
func parseDividConnectedNtfnParams(params []json.RawMessage) (bool, error) {
	if len(params) != 1 {
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance diviutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := diviutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
//
// NOTE: This is a divid extension and requires a websocket connection.
...
//
// NOTE: Deprecated. Use LoadTxFilterAsync instead.
func (c *Client) NotifyReceivedAsync(addresses []diviutil.Address) FutureNotifyReceivedResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
...
//
// NOTE: Deprecated. Use LoadTxFilter instead.
func (c *Client) NotifyReceived(addresses []diviutil.Address) error {
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	addresses []diviutil.Address, outpoints []*wire.OutPoint,
...
// NOTE: This is a divid extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []diviutil.Address,
...
// NOTE: This is a divid extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []diviutil.Address, outPoints []wire.OutPoint) error {

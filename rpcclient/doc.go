...
Bitcoin RPC server that uses a divid/bitcoin core compatible Bitcoin JSON-RPC
API.  This client has been tested with divid (https://github.com/DiviProject/divid),
btcwallet (https://github.com/DiviProject/btcwallet), and
bitcoin core (https://github.com/bitcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, divid and
...
divid or btcwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by divid and
...
All notifications provided by divid require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by divid (and
...
Also, it is important to realize that divid intentionally separates the wallet
functionality into a separate process named btcwallet.  This means if you are
connected to the divid RPC server directly, only the RPCs which are related to

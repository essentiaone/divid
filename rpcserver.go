...
// list of commands that we recognize, but for which divid has no support because
...
func messageToHex(msg wire.Message) (string, error) {
	...
	if err := msg.DiviEncode(&buf, maxProtocolVersion, wire.WitnessEncoding); err != nil {
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount > diviutil.MaxSatoshi {
			return nil, &btcjson.RPCError{
		...
		addr, err := diviutil.DecodeAddress(encodedAddr, params)
		if err != nil {
			return nil, &btcjson.RPCError{
		...
		satoshi, err := diviutil.NewAmount(amount)
		if err != nil {
...
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []btcjson.Vout {
	...
	for i, v := range mtx.TxOut {
		...
		vout.N = uint32(i)
		vout.Value = diviutil.Amount(v.Value).ToDIVI()
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx,
	...
	txReply := &btcjson.TxRawResult{
		...
		Vsize:    int32(mempool.GetTxVirtualSize(diviutil.NewTx(mtx))),
		Weight:   int32(blockchain.GetTransactionWeight(diviutil.NewTx(mtx))),
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	p2sh, err := diviutil.NewAddressScriptHash(script, s.cfg.ChainParams)
	if err != nil {
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	blk, err := diviutil.NewBlockFromBytes(blkBytes)
	if err != nil {
...
//
// NOTE: This is a DiviProject extension originally ported from
...
func handleGetInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &btcjson.InfoChainResult{
		...
		RelayFee:        cfg.minRelayTxFee.ToDIVI(),
	}
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	txOutReply := &btcjson.GetTxOutResult{
		...
		Value:         diviutil.Amount(value).ToDIVI(),
	...
	for _, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &btcjson.PrevOut{
				...
				Value:     diviutil.Amount(originTxOut.Value).ToDIVI(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr diviutil.Address, numToSkip, numRequested uint32) ([]*diviutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	params := s.cfg.ChainParams
	addr, err := diviutil.DecodeAddress(c.Address, params)
	if err != nil {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCInvalidAddressOrKey,
			Message: "Invalid address or key: " + err.Error(),
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	addr, err := diviutil.DecodeAddress(c.Address, s.cfg.ChainParams)
	if err != nil {
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	pk, wasCompressed, err := diviec.RecoverCompact(diviec.S256(), sig,
		expectedMessageHash)
	if err != nil {
	...
	address, err := diviutil.NewAddressPubKey(serializedPK, params)
	if err != nil {
...
//
// NOTE: This is a DiviProject extension ported from github.com/decred/dcrd.
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="divid RPC"`)
...
type rpcserverSyncManager interface {
	...
	SubmitBlock(block *diviutil.Block, flags blockchain.BehaviorFlags) (bool, error)

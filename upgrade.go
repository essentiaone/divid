...
// oldDividHomeDir returns the OS specific home directory divid used prior to
// version 0.3.3.  This has since been replaced with diviutil.AppDataDir, but
// this function is still provided for the automatic upgrade path.
func oldDividHomeDir() string {
	// Search for Windows APPDATA first.  This won't exist on POSIX OSes.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "divid")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".divid")
	}
...
// location prior to divid version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to divid
...
func upgradeDBPaths() error {
	...
	oldDbRoot := filepath.Join(oldDividHomeDir(), "db")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "divid.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "divid_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "divid_regtest.db"), "regtest")

	// Remove the old db directory.
	return os.RemoveAll(oldDbRoot)
}

// upgradeDataPaths moves the application data from its location prior to divid
...
func upgradeDataPaths() error {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		// Create the new path.
		dividLog.Infof("Migrating application home path from '%s' to '%s'",
		...
		// Move old divid.conf into new location if needed.
		...
		if ohpEmpty {
			...
			dividLog.Warnf("Not removing '%s' since it contains files "+
...
// doUpgrades performs upgrades to divid as new versions require it.

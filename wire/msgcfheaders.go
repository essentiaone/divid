...
func (msg *MsgCFHeaders) DiviDecode(r io.Reader, pver uint32, _ MessageEncoding) error {
	// Read filter type
	...
	if count > MaxCFHeadersPerMsg {
		str := fmt.Sprintf("too many committed filter headers for "+
			...
			MaxBlockHeadersPerMsg)
		return messageError("MsgCFHeaders.DiviDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgCFHeaders) DiviEncode(w io.Writer, pver uint32, _ MessageEncoding) error {
	// Write filter type
	...
	if count > MaxCFHeadersPerMsg {
		str := fmt.Sprintf("too many committed filter headers for "+
			"message [count %v, max %v]", count,
			MaxBlockHeadersPerMsg)
		return messageError("MsgCFHeaders.DiviEncode", str)
	}
...
// differs from DiviDecode in that DiviDecode decodes from the bitcoin wire

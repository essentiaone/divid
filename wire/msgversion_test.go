...
func TestVersionWireErrors(t *testing.T) {
	...
	// Ensure calling MsgVersion.DiviDecode with a non *bytes.Buffer returns
	// error.
	fr := newFixedReader(0, []byte{})
	if err := baseVersion.DiviDecode(fr, pver, enc); err == nil {
		t.Errorf("Did not received error when calling " +
			"MsgVersion.DiviDecode with non *bytes.Buffer")
	}
	...
	for i, test := range tests {
		...
		err = msg.DiviDecode(buf, test.pver, test.enc)
...
func TestVersionOptionalFields(t *testing.T) {
	...
	uaVersion.UserAgent = "/dividtest:0.0.1/"

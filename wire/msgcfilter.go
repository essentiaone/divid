...
func (msg *MsgCFilter) DiviDecode(r io.Reader, pver uint32, _ MessageEncoding) error {
	// Read filter type
...
// This is part of the Message interface implementation.
func (msg *MsgCFilter) DiviEncode(w io.Writer, pver uint32, _ MessageEncoding) error {
	size := len(msg.Data)
	if size > MaxCFilterDataSize {
		str := fmt.Sprintf("cfilter size too large for message "+
			"[size %v, max %v]", size, MaxCFilterDataSize)
		return messageError("MsgCFilter.DiviEncode", str)
	}
...
// from DiviDecode in that DiviDecode decodes from the bitcoin wire protocol as

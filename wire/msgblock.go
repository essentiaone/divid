...
func (msg *MsgBlock) DiviDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if txCount > maxTxPerBlock {
		...
		return messageError("MsgBlock.DiviDecode", str)
	}

	msg.Transactions = make([]*MsgTx, 0, txCount)
	for i := uint64(0); i < txCount; i++ {
		tx := MsgTx{}
		err := tx.DiviDecode(r, pver, enc)
		if err != nil {
			return err
		}
		msg.Transactions = append(msg.Transactions, &tx)
...
// Version field in the block.  This function differs from DiviDecode in that
// DiviDecode decodes from the bitcoin wire protocol as it was sent across the
...
func (msg *MsgBlock) Deserialize(r io.Reader) error {
	...
	// a result, make use of DiviDecode.
	//
	// Passing an encoding type of WitnessEncoding to DiviEncode for the
	...
	return msg.DiviDecode(r, 0, WitnessEncoding)
}
...
func (msg *MsgBlock) DeserializeNoWitness(r io.Reader) error {
	return msg.DiviDecode(r, 0, BaseEncoding)
}
...
func (msg *MsgBlock) DiviEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
...
// This function differs from DiviEncode in that DiviEncode encodes the block to
...
func (msg *MsgBlock) Serialize(w io.Writer) error {
	// At the current time, there is no difference between the wire encoding
	// at protocol version 0 and the stable long-term storage format.  As
	// a result, make use of DiviEncode.
	//
	...
	// serialization structure defined in BIP0141.
	return msg.DiviEncode(w, 0, WitnessEncoding)
}
...
func (msg *MsgBlock) SerializeNoWitness(w io.Writer) error {
	return msg.DiviEncode(w, 0, BaseEncoding)
}

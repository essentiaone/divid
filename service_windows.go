...
const (
	// svcName is the name of divid service.
	svcName = "dividsvc"
...
// logServiceStartOfDay logs information about divid when the main server has
...
// dividService houses the main service handler which handles all service
// updates and launching dividMain.
type dividService struct{}
...
// long-running dividMain (which is the real meat of divid), handles service
// change requests, and notifies the service control manager of changes.
func (s *dividService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start dividMain in a separate goroutine so the service can start
...
// installService attempts to install the divid service.  Typically this should
...
// removeService attempts to uninstall the divid service.  Typically this should
...
// startService attempts to start the divid service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &dividService{})
	if err != nil {

...
func Example_signMessage() {
	...
	privKey, pubKey := diviec.PrivKeyFromBytes(diviec.S256(), pkBytes)
...
func Example_verifySignature() {
	...
	pubKey, err := diviec.ParsePubKey(pubKeyBytes, diviec.S256())
	if err != nil {
		fmt.Println(err)
		return
	}
...
func Example_encryptMessage() {
	...
	message := "test message"
	ciphertext, err := diviec.Encrypt(pubKey, []byte(message))
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	privKey, _ := diviec.PrivKeyFromBytes(diviec.S256(), pkBytes)

	// Try decrypting and verify if it's the same message.
	plaintext, err := diviec.Decrypt(privKey, ciphertext)
	if err != nil {
		fmt.Println(err)
		return
	}
